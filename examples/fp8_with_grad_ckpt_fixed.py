#!/usr/bin/env python3
"""
FP8 Full Fine-tuning WITH gradient checkpointing (after fix)

This should now work after the gradient checkpointing fix and use significantly
less memory than fp8_no_grad_ckpt.py (~20GB vs ~77GB for 8B model).
"""
import os
...
print("=" * 80)
print("FP8 Full Fine-tuning (batch=8, seq=512, WITH grad ckpt)")
print("=" * 80)
...
# Training
print("\n" + "=" * 80)
print("Starting FP8 Training (batch=8, seq=512, WITH grad ckpt)")
print("=" * 80)
...
for step, batch in enumerate(dataloader):
    ...
    if (step + 1) % 10 == 0:
        print(f"  Step {step+1}/{num_steps}, Loss: {outputs.loss.item():.4f}, Memory: {torch.cuda.max_memory_reserved() / 1e9:.2f} GB")

elapsed = time.perf_counter() - start

print("\n" + "=" * 80)
print("FP8 Full Fine-tuning Complete (WITH gradient checkpointing)!")
print("=" * 80)
...
print("\nNote: Memory should be ~20GB with grad ckpt vs ~77GB without")
